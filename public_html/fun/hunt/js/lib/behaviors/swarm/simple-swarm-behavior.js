...
var simpleSwarmBehavior = {
    ...
    think: {
        facts: [
            ...
            {
                "name": "hasSpawned", "value": false
            }
            ,
            {
                "name": "canSpawn", "value": false
            }
        ],
        thoughts: [
            {
    ...
    tasks: {
        ...
        move: function (taskExecutionArgs) {
            ...
            if (count % 5 == 0) {
                ...
                var canSpawn = this_agent.recall({ name: "canSpawn" });
                if (canSpawn == true) {
                    var hasSpawned = this_agent.recall({ name: "hasSpawned" });
                    if (hasSpawned != true) {  // only spawn once
                        ...
                        if (Object.keys(this_agent.vicinity.agents).length < 20) {  // only spawn if less than 20 exist
                            ...
                            childDimensions.name = child_agent._name;
                        ...
                        this_agent.remember({ name: "hasSpawned", fact: true });
                    }
                }
            }
        ...
        isClearToGoInDirection: function(conditionArgs) {
            ...
            this_agent.remember({ name: "isClearToGo", fact: true });

            return true;
        },
        isClearToGo: function(conditionArgs) {
            return conditionArgs.agent.getTasks().isClearToGoInDirection(conditionArgs);
        }
    }
};
