 ...
 */
var navigateMazeModerateVisionBehavior = {
    name: "NavigateMaze",
    think: {
        facts: [
            {
    ...
    tasks: {
        attemptMove: function (moveArgs) {
            ...
            if (isClear) {
                ...
                if (this_agent.onMove) {
                    var dimensionsArray = [];
                    var pos = { name: dimensions.name, "x": dimensions.x, "y": dimensions.y, "width": dimensions.width, "height": dimensions.height, color: dimensions.color, image: dimensions.image };
        ...
        move: function (taskExecutionArgs) {
            var currentDirection = taskExecutionArgs.agent.recall({ name: "direction" });
