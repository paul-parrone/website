...
TM_gents__Memory = (function (constructorArgs) {
	return function (constructorArgs) {
        ...
        this._referenceThreshold = constructorArgs.referenceThreshold || 10;
		...
		this.remember = function (name, value) {
			...
			if (fact) {
				fact.referenced++;
				fact.value = value;
                this._attemptLongTerm(name, fact);
				return;
			}
			...
			if (fact) {
				...
				fact = { name: name, value: value, referenced: 1 };
				this._shortTerm.setFact(name, fact);
			}
        ...
        this._attemptLongTerm = function(name, fact) {
            if ((fact.referenced % this._referenceThreshold) == 0) {   // store frequently referenced items in long-term memory
                if (this._longTerm) {
		...
		this.onSerialize = function (serializeArgs) {
			if (this.onPreSerializing({})) {
                ...
                var longTerm = tm_g.stringifyWithFun(this._longTerm);
		...
		this.onDeserialize = function (deserializeArgs) {
            ...
            var longTerm = tm_g.parseWithFun(deserializeArgs.longTerm);

			memory._shortTerm = shortTerm;
            memory._longTerm = longTerm;
			return memory;
		};
	}
	...
	return function (constructorArgs) {
		...
		this._removeLeastUsedItems = function () {
			var arr = this._cache.sort(function (first, second) {
				var sts = first.referenced - second.referenced;
...
TM_gents__LongTermMemory = (function (constructorArgs) {

    return function (constructorArgs) {
        this._memoryName = constructorArgs.memoryName || "agt.mem";
        this._cache = [];

        this._getData = function () {
            if (typeof(localStorage) != "undefined") {
                var longTermData = localStorage[this._memoryName];
                this._cache = (longTermData) ? JSON.parse(longTermData) : this._cache;
            }
        ...
        this._setData = function () {
            if (typeof(localStorage) != "undefined") {
                localStorage[this._memoryName] = JSON.stringify(this._cache);
            }

        };

        this.setFact = function (name, inFact) {
            ...
            if (!fact) {
                ...
                var storedFact = tm_g.findOne(this._cache, function (value) {
                    var item = value;
                ...
                storedFact.value = inFact.value;
                storedFact.referenced++;
                storedFact.lastAccessed = fact.lastAccessed;
            }
		...
		this.setThoughts = function (agentName, newThoughts, newTasks) {
			...
			tm_g.each(newThoughts, function (value, key) {
				...
				if (!tm_g.isFunction(thght.onCondition)) {
                    thght.onConditionName = thght.onCondition;
	...
	return function (constructorArgs) {
		...
		this.scheduleTask = function (taskProcessArgs) {
			if (taskProcessArgs.agent && taskProcessArgs.agent.isAlive()) {
				// schedule the task
				var ths = this;
				ths.currentTaskNumber = setTimeout(function (taskProcessArgs) {
				...
				}, taskProcessArgs.delay, taskProcessArgs);
		...
		this.processTask = function (taskProcessArgs) {
            if (!taskProcessArgs.agent.isAlive()) {
                return;
            }

            var vicinity = taskProcessArgs.agent.vicinity;
            vicinity.agentPing(taskProcessArgs.agent);
...
TM_gents__Agent = (function (constructorArgs) {
	return function (constructorArgs) {
		...
		this.notice = function (noticeArgs) {
			...
			noticeArgs.source = this;
			var pkg = { data:  noticeArgs };
		...
		this.say = function(messageArgs) {
            var type = typeof messageArgs;
            if (type == "string") {     // ex. say("hello");
                var msgArgs = {
                    name: "msg",
                    args: {
                        "body": {
                ...
                return this.postMsg(msgArgs)
            }

            if (!messageArgs.name) {    // ex. say({body: { text: "hello"}});
                var msgArgs = {
                    name: "msg",
                    args: messageArgs
                };
			...
			return this.postMsg(messageArgs);   // ex. say({name:"msg" {body: { text: "hello"}}});
		};

		this.reply = function(originalMessageArgs, text, ttl) {
                var msgArgs = {
                    name: "msg",
                    args: {
                        header: {
        ...
        this.replyAttachment = function(originalMessageArgs, attachment, ttl) {
            var msgArgs = {
                name: "msg",
                args: {
                    header: {
        ...
        this.getBehaviors = function() {
            ...
            for (var key in behaviors) {
                list.push({behavior: behaviors[key]});
            }
		...
		this.spawn = function(name,makeLive) {
			...
			if (this.vicinity) {
                ...
                var agtParts = this.serialize();
				var childAgt = this.deserialize(agtParts);
            ...
            childAgt.vicinity = this.vicinity;
            childAgt.vicinity.addAgent(childAgt);

            var tasksObj = childAgt._tasks;
			var thoughts = childAgt._thoughts._thoughts;
			for (var i=0; i<thoughts.length; i++) {     // ASSUMPTION: all the same order
                var thought = thoughts[i];
				thought.agentName = childName;
                if (thought.onConditionName) {
                    thought.onCondition = tasksObj[thought.onConditionName];
                }
			}

            // update tasks
            var tasks = tasksObj._tasks;
            for (var i=0; i<tasks.length; i++) {
                tasks[i].agentName = childName;
            }
...
TM_gents__Agent_getAgent = function(agentArgs) {
	...
	var mem = (agentArgs.memory) ? agentArgs.memory : new TM_gents__Memory({ longTerm: null,
                                                            referenceThreshold: agentArgs.referenceThreshold });
    ...
    agt._originalPulseInterval = agt._pulseInterval;
	if (agentArgs.pulseDuration) {agt._pulseDuration = agentArgs.pulseDuration};
    agt._originalPulseDuration = agt._pulseDuration;
    ...
    agt.onNotice = agentArgs.onNotice;
    agt.onLearnAll = agentArgs.onLearnAll;
    agt.onLearn = agentArgs.onLearn;
