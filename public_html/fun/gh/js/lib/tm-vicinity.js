...
TM_gents__Vicinity.getVicinity = function(vicinityArgs) {
    ...
    // add port 49999 if a worker process
...
TM_gents__Vicinity.prototype.stopAgents = function(agent) {
...
TM_gents__Vicinity.prototype.connect = function(connectionArgs) {
    ...
    if (this._connections[connectorUrl]) {  // already connected then ignore
        return this._connections[connectorUrl];
    }
                ...
                for (var key in this._connections) {
                    ...
                    if (conn.status == "Open") {
                        ...
                        if (!this._isOrigin(cmd.args, conn._url)) {
                            if (this._isATarget(cmd.args, conn._url)) {
...
TM_gents__Vicinity.prototype._initMessage = function(args) {
    ...
    if (!header.ttl == undefined) {header.ttl = 1}
...
TM_gents__Vicinity.prototype._isATarget = function(messageArgs, connUrl) {
    if (this._isProxy) {
...
TM_gents__ServerProxyConnection.prototype.setupConnection = function(connectionArgs) {
	...
	var connectInfo = {
		args: {
			...
			groups: this._connector.config.groups
		}
	};
